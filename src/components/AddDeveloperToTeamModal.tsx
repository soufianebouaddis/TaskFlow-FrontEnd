...
import { Code, Mail, X, UserPlus } from 'lucide-react';
...
interface AddDeveloperToTeamModalProps {
  developers: Developer[];
  onAddToTeam: (developerId: string) => void;
  onClose: () => void;
  isLoading: boolean;
}

const AddDeveloperToTeamModal: React.FC<AddDeveloperToTeamModalProps> = ({
  developers,
  onAddToTeam,
  onClose,
}) => {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-8 w-full max-w-md">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-semibold text-white">Add Developer to Team</h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-6">
          <p className="text-slate-300 text-sm mb-4">
            Select a developer to add to your team:
          </p>
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {developers.map((developer) => (
              <div
                key={developer.id}
                onClick={() => onAddToTeam(developer.id)}
...
export default AddDeveloperToTeamModal; 